...
export function activate(context: vscode.ExtensionContext) {
  ...
  const diagnosticDisposable = vscode.commands.registerCommand(
    ...
    async (uri?: vscode.Uri) => {
      console.log(
        "=== DIAGNOSTIC COMMAND: Starting comprehensive diagnostic ==="
      );
      ...
      try {
        console.log(
          "DIAGNOSTIC: Running comprehensive diagnostic test on:",
          fileUri.fsPath
        );
        ...
        const summary = [
          ...
          "Parser Results:",
          `• Simple Parser: ${
            result.simpleParserResult?.success ? "✓ SUCCESS" : "✗ FAILED"
          }`,
          `• Complex Parser: ${
            result.complexParserResult?.success ? "✓ SUCCESS" : "✗ FAILED"
          }`,
          `• Flexible Parser: ${
            result.flexibleParserResult?.success ? "✓ SUCCESS" : "✗ FAILED"
          }`,
          "",
          "Data Structure Differences:",
          ...result.dataStructureDifferences.map(
            (diff) =>
        ...
        console.error(
          "DIAGNOSTIC: Error during comprehensive diagnosis:",
          error
        );
  ...
  const disposable = vscode.commands.registerCommand(
    ...
    async (uri?: vscode.Uri) => {
      console.log(
        "=== HYBRID PARSER: claimsTimeline.viewTimeline command started ==="
      );
      console.log(
        "Command triggered with URI:",
        uri?.fsPath || "no URI provided"
      );

      const fileUri = uri || vscode.window.activeTextEditor?.document.uri;
      console.log(
        "Resolved file URI:",
        fileUri?.fsPath || "no file URI available"
      );

      if (!fileUri) {
        console.log(
          "HYBRID PARSER: No JSON file selected - showing error message"
        );
      ...
      try {
        console.log(
          "HYBRID PARSER: Starting hybrid parsing approach for:",
          fileUri.fsPath
        );
        ...
        console.log(
          `HYBRID PARSER: Parsing successful with ${timelineData.claims.length} claims`
        );
        ...
        if (timelineData.claims.length === 0) {
          console.log(
            "HYBRID PARSER: No claims found - showing warning message"
          );
        ...
        console.error(
          "HYBRID PARSER: Error occurred in claimsTimeline.viewTimeline command:",
          error
        );
        ...
        const errorMessage =
          error instanceof Error ? error.message : "Unknown error";
        vscode.window
          .showErrorMessage(`Timeline Error: ${errorMessage}`, "Show Details")
          .then((selection) => {
      ...
      console.log(
        "=== HYBRID PARSER: claimsTimeline.viewTimeline command completed ==="
      );
    }
  );

  // Register test parsing command
  const testParsingDisposable = vscode.commands.registerCommand(
    ...
    async (uri?: vscode.Uri) => {
      ...
      if (!fileUri) {
        vscode.window.showErrorMessage(
          "No JSON file selected for parsing test"
        );
        return;
      }
      ...
      try {
        console.log(
          "TEST PARSING: Testing parsing capabilities on:",
          fileUri.fsPath
        );
        ...
        console.log(
          `TEST PARSING: Claims found: ${timelineData.claims.length}`
        );
          ...
          ...timelineData.claims
            ...
            .map(
              (claim, index) =>
                `${index + 1}. ${
                  claim.displayName
                } (${claim.startDate.toLocaleDateString()} - ${claim.endDate.toLocaleDateString()})`
            ),
          timelineData.claims.length > 3
            ? `... and ${timelineData.claims.length - 3} more claims`
            : "",
          "",
  ...
  const debugInfoDisposable = vscode.commands.registerCommand(
        ...
        if (fileUri) {
          try {
            ...
            fileInfo = {
              ...
              rxTbaCount:
                jsonValid && jsonData?.rxTba ? jsonData.rxTba.length : "N/A",
              rxHistoryCount:
                jsonValid && jsonData?.rxHistory
                  ? jsonData.rxHistory.length
                  : "N/A",
              medHistoryCount:
                jsonValid && jsonData?.medHistory
                  ? jsonData.medHistory.length
                  : "N/A",
            };
        ...
        const debugInfo = [
          ...
          ...(fileUri
            ? [
                ...
                `• Top-level Keys: ${
                  (fileInfo as any).topLevelKeys?.join(", ") || "N/A"
                }`,
                ...
                ...((fileInfo as any).error
                  ? [`• Error: ${(fileInfo as any).error}`]
                  : []),
              ]
            : ["• No file selected"]),
          "",
          ...
          `• Diagnostic Logging: ${
            configInfo.enableDiagnosticLogging ? "Enabled" : "Disabled"
          }`,
          `• Default Date Format: ${configInfo.defaultDateFormat}`,
          `• Custom Colors: ${
            Object.keys(configInfo.customColors).length > 0
              ? "Configured"
              : "None"
          }`,
